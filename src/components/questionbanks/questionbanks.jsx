...
class QuestionBanks extends Component {
  ...
  async fetchData() {
    const response = await fetch(
      process.env.REACT_APP_API_URI + "/questionbanks/get",
      {
        method: "POST",
        headers: {
    ...
    if (data.message) {
      //if we dont get questionbanks from server but got a message
  ...
  async postNewQuestionBank() {
    const newQuestionBankName = document.getElementById(
      "newQuestionBankInput"
    ).value;
    ...
    if (newQuestionBankName !== "") {
      const response = await fetch(
        process.env.REACT_APP_API_URI + "/questionbanks/new",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async deleteQuestionBank(id, questionBankName) {
    var confirm = window.confirm(
      "Confirm to Delete the Question Bank: " + questionBankName
    );

    if (confirm) {
      const response = await fetch(
        process.env.REACT_APP_API_URI + "/questionbanks/del",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    return (
      <div>
          ...
          {this.state.loading
            ...
            : this.state.questionBanks.map((e, indx) => {
                return (
                  <SmallCard
                    key={indx}
                    href={"/questionbanks/" + e._id}
                    header={e.questionBankName}
                    footer={`${e.questions.length} ${
                      e.questions.length > 1 ? " questions" : " question"
                    }`}
                    deleteHandler={async () =>
                      await this.deleteQuestionBank(e._id, e.questionBankName)
                    }
                  />
                );
              })}
        ...
        <div className={stylesCSS.addNewContainer}>
          <AddNewCard
            inputId="newQuestionBankInput"
            placeholder="New Question Bank"
            onClick={this.postNewQuestionBank}
          />
        </div>
